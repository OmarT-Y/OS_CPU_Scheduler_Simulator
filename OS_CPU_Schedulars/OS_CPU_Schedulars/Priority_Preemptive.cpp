...
int Priority_P_Sch::run_Schedular(int System_time, int prev_running_time)
{
	...
	if (System_time == 0) //Initial System State
	{
		...
		if (!Ready_Q.empty()) 
		{
			...
			return min(Running_task->remaining_burst, ( Blocked_Q[0]->ariv_time - System_time));
		}
	}
	...
	else //a new process got added or idle process
	{
		...
		{
			if (Running_task == Ready_Q[0])	//if the highest priority is the running task
			{
				Running_task->remaining_burst += (expected_last_return - prev_running_time);
				return min(Running_task->remaining_burst, (Blocked_Q[0]->ariv_time - System_time));
			}
			else	//realocate the CPU
			{
				Running_task->remaining_burst += (expected_last_return - prev_running_time);
				Running_task = Ready_Q[0];
